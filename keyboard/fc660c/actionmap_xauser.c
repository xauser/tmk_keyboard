...
const action_t PROGMEM actionmaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    // gaming layer
    KMAP(
        ESC, TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,BSPC,     INS,
        TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,LBRC,RBRC,BSLS,     DEL,
...
void hook_layer_change(uint32_t layer_state) {
    if (layer_state & (1 << LAYER_GAMING)) {
        PORTB &= ~(1 << 5);
    } else {
        PORTB |=  (1 << 5);
    }
    if (layer_state & (1 << LAYER_NAV)) {
        PORTB &= ~(1 << 6);
    } else {
        PORTB |=  (1 << 6);
    }
